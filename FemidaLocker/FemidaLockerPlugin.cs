...
namespace FemidaLocker
{
    [ApiVersion(2, 1)]
    [UsedImplicitly]
    public sealed class FemidaLockerPlugin : TerrariaPlugin
    {
        /// <summary>
        ...
        private static readonly string ConfigPath = Path.Combine("femida locker", "config.json");
        ...
        public override string Author => "Commit & blick";
        public override string Description => "Provides anti-hack measures.";
        public override string Name => "FemidaLocker";
        public override Version Version => Assembly.GetExecutingAssembly().GetName().Version;

        public override void Initialize()
        {
            Directory.CreateDirectory("FemidaLocker");
            if (File.Exists(ConfigPath))
            {
                FemidaLockerConfig.Instance = JsonConvert.DeserializeObject<NoCheatConfig>(File.ReadAllText(ConfigPath));
            }
        ...
        private void FemidaLokcer(CommandArgs args)
        {
            var parameters = args.Parameters;
            var subcommand = parameters.Count > 0 ? parameters[0] : "";
            if (subcommand.Equals("list", StringComparison.OrdinalIgnoreCase))
            {
                FemidaLockerList(args);
            }
            else if (subcommand.Equals("pardon", StringComparison.OrdinalIgnoreCase))
            {
                FemidaLockerPardon(args);
            }
            else
            {
                var player = args.Player;
                player.SendErrorMessage("Syntax: /femidalocker list <player/user> [page]");
                player.SendErrorMessage("Syntax: /femidalocker pardon <player/user> <number>");
            }
        }
        ...
        private void NoCheatList(CommandArgs args)
        {
            ...
            if (players.Count == 0)
            {
                var path = Path.Combine("FemidaLocker", $"{inputPlayerOrUser}.session");
                if (!File.Exists(path))
                {
